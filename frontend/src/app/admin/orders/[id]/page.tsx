...
export default function OrderDetailsPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    const onBusSubmit = (data: any) => {
        ...
        formData.append('busDetails[busNumber]', data.busNumber);
        formData.append('busDetails[driverContact]', data.driverContact);
        formData.append('busDetails[dispatchDate]', data.dispatchDate);
        formData.append('busDetails[departureTime]', data.departureTime);
        formData.append('busDetails[expectedArrival]', data.expectedArrival);
                    ...
                    <div style={{
                        ...
                        <form onSubmit={handleSubmit(onBusSubmit)} style={{ padding: '1.5rem' }}>
                                ...
                                <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                                    ...
                                    <div className="form-group">
                                        ...
                                        <div style={{ position: 'relative' }}>
                                            ...
                                            <input {...register("driverContact", { required: true })} className="form-input" style={{ paddingLeft: '2.5rem' }} placeholder="+91 Driver Number" />
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div style={{ margin: '1.5rem 0' }}>
                                ...
                                <label className="upload-box" style={{ height: '160px', padding: 0, overflow: 'hidden' }}>
                                    {busPhotoPreview ? (
                                        <div style={{ width: '100%', height: '100%', position: 'relative' }}>
                                            <Image src={busPhotoPreview} fill style={{ objectFit: 'contain', padding: '10px' }} alt="Preview" />
                                            <div style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,0.4)', opacity: 0, transition: '0.2s', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
                                                onMouseEnter={(e) => e.currentTarget.style.opacity = '1'}
                                                onMouseLeave={(e) => e.currentTarget.style.opacity = '0'}
                                            >
                                                <span className="badge" style={{ background: 'white', color: 'black' }}>Change Image</span>
                                            </div>
                                        </div>
                                    ) : (
                                        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '2rem' }}>
                                            <FiUploadCloud size={32} style={{ color: 'var(--primary)', marginBottom: '0.5rem' }} />
                                            <span style={{ fontWeight: 600 }}>Click to Upload</span>
                                            <span style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}>JPG/PNG Max 5MB</span>
                                        </div>
                                    )}
                                    <input type="file" {...register("busPhoto")} accept="image/*" />
                                </label>
                            </div>
                            ...
                            <div style={{ padding: '1.5rem', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '2rem' }}>
                                    ...
                                    <div>
                                        ...
                                        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginTop: '0.5rem' }}>
                                            <div style={{ background: '#F8FAFC', padding: '0.5rem 1rem', borderRadius: '6px', border: '1px solid var(--border)' }}>
                                                ...
                                                <div style={{ fontWeight: 600 }}>{order.busDetails.departureTime || '--:--'}</div>
                                            </div>
                                ...
                                <div style={{ height: '100%', minHeight: '120px', background: '#F1F5F9', borderRadius: '8px', overflow: 'hidden', position: 'relative', border: '1px solid var(--border)' }}>
                                    {order.busDetails.busPhoto ? (
                                        <a href={`http://localhost:5000/${order.busDetails.busPhoto}`} target="_blank" style={{ display: 'block', height: '100%', width: '100%' }}>
                                            <Image src={`http://localhost:5000/${order.busDetails.busPhoto}`} fill style={{ objectFit: 'cover' }} alt="Proof" />
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                            {['Packed', 'Assigned to Bus', 'Delivered'].map(status => (
                                <button
                                    ...
                                    disabled={processing || order.status === 'Cancelled' || order.status === status}
                                    style={{
                                        ...
                                        borderRadius: '8px', cursor: (order.status === status || order.status === 'Cancelled') ? 'default' : 'pointer',
